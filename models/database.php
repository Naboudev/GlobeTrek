...
function ajouterReservation($reference, $date_arrivee, $date_depart, $nombre_nuits, $nombre_personnes, $demandes_speciales, $methode_paiement, $date_reservation, $chambre_id, $client_id, $prix, $actions = 0){
    global $db;
    try {
...
function mesReservations($client_id){
    ...
    try {
        $q = $db->prepare("
            ...
            AND r.actions = :status
            ORDER BY r.date_reservation DESC

        ");
        $q->execute([
        "client_id" => $client_id,
        "status" => 0 // ou 1, 2 selon le filtre souhaité
]);
